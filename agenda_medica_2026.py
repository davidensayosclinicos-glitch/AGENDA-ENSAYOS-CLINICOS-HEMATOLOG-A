...
def parse_fecha_iso(fecha_iso):
    ...
    if "T" in valor:
        try:
            valor_iso = valor[:-1] + "+00:00" if valor.endswith("Z") else valor
            dt = datetime.fromisoformat(valor_iso)
            if dt.tzinfo is not None:
                # Convertir a hora local para obtener la fecha del usuario, no la UTC.
                dt = dt.astimezone(ZoneInfo("Europe/Madrid")) if ZoneInfo is not None else dt.astimezone()
            return dt.date()
        except (ValueError, TypeError):
            pass
        ...
        com["items_unificados"] = [
            {
                ...
                "relacion_con": it.get("relacion_con", ""),
                "historia_medica": it.get("historia_medica", ""),
    ...
    def _render_unificado_clinico(estado, visita_id):
        ...
        for it in items:
            ...
            dedup.append({
                ...
                "relacion_con": str(it.get("relacion_con", "") or ""),
                "historia_medica": str(it.get("historia_medica", "") or ""),
